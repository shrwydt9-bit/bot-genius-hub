 ...
 serve(async (req) => {
   ...
   try {
     ...
     console.log("Email webhook payload:", JSON.stringify(body));
     ...
     // Extract email content (format may vary by email service provider)
     const userMessage = body.text || body.plain || body.stripped_text || "";
     const fromEmail = body.from || body.sender;
 
     if (userMessage && fromEmail) {
       console.log(`Email from ${fromEmail}: ${userMessage}`);
 
       const apiKey = await createOpenRouterClient();
       const systemPrompt = `You are a ${deployment.bots.personality || "professional"} email assistant. ${deployment.bots.description || ""}`;
       ...
       // Email reply would be sent via SMTP or email service API
       // Store credentials in deployment.config
     }
