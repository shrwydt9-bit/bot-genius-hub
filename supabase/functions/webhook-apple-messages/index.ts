 ...
 serve(async (req) => {
   ...
   try {
     ...
     console.log("Apple Messages webhook payload:", JSON.stringify(body));
     ...
     const sender = body.sourceId || body.from;
 
     if (userMessage && sender) {
       console.log(`Apple Messages from ${sender}: ${userMessage}`);
 
       const apiKey = await createOpenRouterClient();
       const systemPrompt = `You are a ${deployment.bots.personality || "professional"} Apple Messages for Business assistant. ${deployment.bots.description || ""}`;
     ...
     console.error("Apple Messages webhook error:", error);
     return new Response(
