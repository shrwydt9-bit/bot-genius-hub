 ...
 serve(async (req) => {
   ...
   try {
     ...
     console.log("LinkedIn webhook payload:", JSON.stringify(body));
     ...
     const sender = body.sender?.id || body.from;
 
     if (userMessage && sender) {
       console.log(`LinkedIn message from ${sender}: ${userMessage}`);
 
       const apiKey = await createOpenRouterClient();
       const systemPrompt = `You are a ${deployment.bots.personality || "professional"} LinkedIn assistant for business networking. ${deployment.bots.description || ""}`;
