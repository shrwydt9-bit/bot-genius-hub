 ...
 serve(async (req) => {
   ...
   try {
     ...
     console.log("Kik webhook payload:", JSON.stringify(body));
     ...
     for (const msg of messages) {
       if (msg.type === "text") {
         ...
         console.log(`Kik message from ${sender}: ${userMessage}`);
 
         const apiKey = await createOpenRouterClient();
         const systemPrompt = `You are a ${deployment.bots.personality || "friendly"} Kik bot. ${deployment.bots.description || ""}`;
     ...
     console.error("Kik webhook error:", error);
     return new Response(
