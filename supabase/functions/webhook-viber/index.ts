 ...
 serve(async (req) => {
   ...
   try {
     ...
     console.log("Viber webhook payload:", JSON.stringify(body));
     ...
     if (body.event === "message") {
       ...
       if (userMessage && sender) {
         console.log(`Viber message from ${sender}: ${userMessage}`);
 
         const apiKey = await createOpenRouterClient();
         const systemPrompt = `You are a ${deployment.bots.personality || "friendly"} Viber assistant. ${deployment.bots.description || ""}`;
