 ...
 serve(async (req) => {
   ...
   try {
     ...
     console.log("WeChat webhook payload:", JSON.stringify(body));
     ...
     const userMessage = body.Content || body.text;
     const sender = body.FromUserName || body.from;
 
     if (userMessage && sender) {
       console.log(`WeChat message from ${sender}: ${userMessage}`);
 
       const apiKey = await createOpenRouterClient();
       const systemPrompt = `You are a ${deployment.bots.personality || "helpful"} WeChat assistant. ${deployment.bots.description || ""}`;
