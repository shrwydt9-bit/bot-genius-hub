   ...
   try {
     ...
     for (const event of events) {
       if (event.type === "message" && event.message?.type === "text") {
         ...
         console.log(`Line message from ${sender}: ${userMessage}`);
 
         const apiKey = await createOpenRouterClient();
         const systemPrompt = `You are a ${deployment.bots.personality || "friendly"} Line assistant. ${deployment.bots.description || ""}`;
