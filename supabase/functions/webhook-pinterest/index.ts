 ...
 serve(async (req) => {
   ...
   try {
     ...
     console.log("Pinterest webhook payload:", JSON.stringify(body));
     ...
     const userMessage = body.comment?.text || body.message;
     const sender = body.user?.username || body.from;
 
     if (userMessage && sender) {
       console.log(`Pinterest comment from ${sender}: ${userMessage}`);
 
       const apiKey = await createOpenRouterClient();
       const systemPrompt = `You are a ${deployment.bots.personality || "creative"} Pinterest assistant. ${deployment.bots.description || ""}`;
