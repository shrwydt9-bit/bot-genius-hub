 ...
 serve(async (req) => {
   ...
   try {
     ...
     console.log("RCS webhook payload:", JSON.stringify(body));
     ...
     const sender = body.senderPhoneNumber || body.from;
 
     if (userMessage && sender) {
       console.log(`RCS message from ${sender}: ${userMessage}`);
 
       const apiKey = await createOpenRouterClient();
       const systemPrompt = `You are a ${deployment.bots.personality || "helpful"} RCS Business Messaging assistant. ${deployment.bots.description || ""}`;
