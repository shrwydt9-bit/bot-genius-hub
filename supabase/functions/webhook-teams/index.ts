 ...
 serve(async (req) => {
   ...
   try {
     ...
     console.log("Microsoft Teams webhook payload:", JSON.stringify(body));
     ...
     const userMessage = body.text || body.value?.text;
     const sender = body.from?.name || body.user;
 
     if (userMessage && sender) {
       console.log(`Teams message from ${sender}: ${userMessage}`);
 
       const apiKey = await createOpenRouterClient();
       const systemPrompt = `You are a ${deployment.bots.personality || "professional"} Microsoft Teams assistant for workplace collaboration. ${deployment.bots.description || ""}`;
