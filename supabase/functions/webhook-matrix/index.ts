 ...
 serve(async (req) => {
   ...
   try {
     ...
     console.log("Matrix webhook payload:", JSON.stringify(body));
     ...
     const userMessage = body.content?.body || body.text;
     const sender = body.sender || body.from;
 
     if (userMessage && sender && body.type === "m.room.message") {
       console.log(`Matrix message from ${sender}: ${userMessage}`);
 
       const apiKey = await createOpenRouterClient();
       const systemPrompt = `You are a ${deployment.bots.personality || "decentralized"} Matrix bot. ${deployment.bots.description || ""}`;
