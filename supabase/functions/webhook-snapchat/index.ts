 ...
 serve(async (req) => {
   ...
   try {
     ...
     console.log("Snapchat webhook payload:", JSON.stringify(body));
     ...
     const userMessage = body.message?.text || body.snap?.text;
     const sender = body.sender?.username || body.from;
 
     if (userMessage && sender) {
       console.log(`Snapchat message from ${sender}: ${userMessage}`);
 
       const apiKey = await createOpenRouterClient();
       const systemPrompt = `You are a ${deployment.bots.personality || "fun and casual"} Snapchat assistant. ${deployment.bots.description || ""}`;
