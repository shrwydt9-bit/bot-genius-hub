 ...
 serve(async (req) => {
   ...
   try {
     ...
     const userMessage = body.comment?.text || body.message?.text;
     const sender = body.user?.username || body.from_user;
 
     if (userMessage && sender) {
       console.log(`TikTok message from ${sender}: ${userMessage}`);
 
       const apiKey = await createOpenRouterClient();
       const systemPrompt = `You are a ${deployment.bots.personality || "fun and energetic"} TikTok assistant. Keep responses short and engaging. ${deployment.bots.description || ""}`;
