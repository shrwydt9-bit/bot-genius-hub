 ...
 serve(async (req) => {
   ...
   try {
     ...
     console.log("YouTube webhook payload:", JSON.stringify(body));
     ...
     const userMessage = body.snippet?.topLevelComment?.snippet?.textDisplay || body.text;
     const sender = body.snippet?.authorDisplayName || body.author;
 
     if (userMessage && sender) {
       console.log(`YouTube comment from ${sender}: ${userMessage}`);
 
       const apiKey = await createOpenRouterClient();
       const systemPrompt = `You are a ${deployment.bots.personality || "engaging"} YouTube comment assistant. ${deployment.bots.description || ""}`;
