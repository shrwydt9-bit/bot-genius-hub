 ...
 serve(async (req) => {
   ...
   try {
     ...
     console.log("Signal webhook payload:", JSON.stringify(body));
     ...
     const userMessage = body.dataMessage?.message || body.text;
     const sender = body.source || body.from;
 
     if (userMessage && sender) {
       console.log(`Signal message from ${sender}: ${userMessage}`);
 
       const apiKey = await createOpenRouterClient();
       const systemPrompt = `You are a ${deployment.bots.personality || "secure and private"} Signal bot focused on privacy. ${deployment.bots.description || ""}`;
